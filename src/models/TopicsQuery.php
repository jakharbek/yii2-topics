...
namespace jakharbek\topics\models;
...
class TopicsQuery extends \yii\db\ActiveQuery
{
    ...
    */
    public function buildTreeByRoot($keys = [],$type = null)
    {
        $i = 0;
        if($type == null){
            $roots = Topics::find()->lang()->roots()->asArray()->all();
        }else{
            $roots = Topics::find()->lang()->type($type)->roots()->asArray()->all();
        }
    ...
    public function childrensById($id = null,$keys = []){
        $i = 0;
        $count = Topics::findOne($id)->children(1)->count();
        if($count == 0){return [];}
        $data = [];
        $childs = Topics::findOne($id)->children(1)->asArray()->all();
